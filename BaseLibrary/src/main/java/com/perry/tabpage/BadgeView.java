 ...
 * 一个简单的文本标签视图,可以作为一个“徽章”适用于任何给定{ @link android.view.View }。这个类的目的是在运行时被实例化,而不是包含在XML布局。
 *
 * @author perry.li
 */
public class BadgeView extends TextView {
    ...
    private void applyLayoutParams() {
        ...
        if (badgewidth > 0 && badgeheight > 0) {
            lp = new FrameLayout.LayoutParams(badgewidth, badgeheight);
        }
        if (null == lp) {
    ...
    public void setBadgewidth(int badgewidth) {
        this.badgewidth = dipToPixels(badgewidth);
    }
    ...
    public void setBadgeheight(int badgeheight) {
        this.badgeheight = dipToPixels(badgeheight);
    }
